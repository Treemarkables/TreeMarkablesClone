...
- **Phase 2 — soak window started (2026-05-11):**
  ...
  - **Cloudflare proxy drift + cache risk (2026-05-11):** while debugging the photo upload, a response from `do.app.treemarkables.co.nz/objects/photos/...` returned `Server: cloudflare` and `Cf-Cache-Status: HIT` — meaning the `do.app` CNAME had been flipped from grey-cloud (DNS only) to orange-cloud (proxied) at some point after the original Phase 2 DNS setup. Plan called for grey-cloud (line 181). Reverted to grey-cloud + ran *Caching → Configuration → Purge Everything* in Cloudflare to clear cached responses (max-age was 1 year per `photoStorage.ts:368-370` and similar). **Phase 4 + 5 follow-up:** `photoStorage.ts` sets long `Cache-Control: public, max-age=31536000` headers on photo responses intending *browser* caching. If a CDN ever sits in front of `app.treemarkables.co.nz`, those responses are shared/private without cache-key isolation — one user's photo could be served to another from edge cache. Either (a) keep CDNs in DNS-only mode for the app hostname, or (b) switch to `Cache-Control: private, max-age=...` server-side before introducing a CDN. Decision deferred to Phase 5.
