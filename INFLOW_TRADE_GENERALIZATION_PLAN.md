...
- **Customer-facing document viewers** — public, session-less, so identity must ride in the payload. Pattern established: NEW `storage.getDefaultDocumentTemplateForBusiness(businessId, type)` resolves the branding template for the DOCUMENT's tenant (ownerDb, falls back to unscoped default for TM legacy rows). PARTIALLY DONE:
  ...
  - **Invoice view** (`/api/invoices/:id/public` → `InvoiceView.tsx`) — NOT done, and ⚠️ **higher stakes than branding**: it renders a hardcoded **bank account** (`Treemarkables Ltd` / `06-0637-0768850-00`) — every tenant's invoice currently tells the customer to pay into TM's bank account (misdirected payments). **There is NO per-tenant bank field in the schema.** Needs: a product decision (bank-transfer fields vs the existing Stripe pay-link), then new `business_settings` bank columns + UI + payload + viewer. Treat as its own focused PR.
