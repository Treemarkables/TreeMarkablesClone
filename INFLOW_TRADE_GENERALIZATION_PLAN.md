...
- ~~**GST number (email path)**~~ — DONE: added `businessSettings.businessGstNumber` (migration `20260627_business_gst_number.sql`, TM seeded by name) + `getBusinessIdentity().gstNumber` (default `''`, **no TM fallback**). Branded-email footer now renders the GST line only when set, so a new tenant never shows TM's GST.
- ~~**Business-identity Settings UI**~~ — DONE: extended `SettingsCompany.tsx` ("Company Info") with an "Emails & AI" section editing `business_settings` (brand name / owner / tagline / trade) via `PUT /api/business-settings`. GST is one input dual-written to BOTH stores (document_templates + business_settings) so invoices and emails never disagree. **Note the two-store split is intentional** for now: `document_templates` = legal/PDF branding, `business_settings` = email/AI identity (brand wordmark ≠ legal name). **Still TODO:** PDF GST still reads `document_templates.gstNumber` (`routes.ts:1334/1543`) — fine, the UI keeps both in sync; full store unification is a later consolidation.
