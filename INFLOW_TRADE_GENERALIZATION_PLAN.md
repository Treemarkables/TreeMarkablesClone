...
- **Customer-facing document viewers** — public, session-less, so identity must ride in the payload. Pattern established: NEW `storage.getDefaultDocumentTemplateForBusiness(businessId, type)` resolves the branding template for the DOCUMENT's tenant (ownerDb, falls back to unscoped default for TM legacy rows). PARTIALLY DONE:
  - ~~**Proposal accept**~~ (`/api/proposals/:id/public` → `ProposalAccept.tsx`) — DONE: template scoped to `proposal.businessId`; server + client TM-literal fallbacks neutralized to blank. Verified on dev (TM template carries its business_id → unchanged).
  - ~~**Invoice view**~~ (`/api/invoices/:id/public` → `InvoiceView.tsx`) — DONE (per-tenant bank-transfer fields chosen). NEW `business_settings.bankAccountName`/`bankAccountNumber` (migration `20260627_business_bank_details.sql`, TM seeded by name) + `storage.getBusinessSettingsForBusiness(businessId)`. Public invoice payload now carries a `company` object (contact from the tenant's document template, bank from its settings, scoped to `invoice.businessId`); `InvoiceView.tsx` renders the **payment block only when the bank account is set** — so a tenant who hasn't configured one shows nothing, never TM's account. Footer/contact/logo-alt de-hardcoded too. Verified on dev: TM seeded + unchanged, real tenant "Cut right" shows no bank block. **Still TODO:** a Settings UI for tenants to enter their bank details (fold into the Company Info bank/payment section — currently set via DB/onboarding).
  - Authenticated previews (`ProposalViewer`/`QuoteViewer`/`InvoiceViewer`/`CustomerPortal`) — opened logged-in; can read identity from `/api/business-settings` or the same payload. Lower risk; follow-up.
...
- ~~**Branded-email header**~~ — DONE: the `<TREEMARKABLES>` wordmark + "Professional Arborists · Gisborne" subtitle were fully hardcoded (ignored `opts.company`). Now per-tenant: wordmark = business name uppercased via CSS (TM renders "TREEMARKABLES" byte-identically), subtitle = `businessTagline`. `server/emailTemplates.ts` + the 3 `renderBrandedEmail` callers.
- ~~**GST number (email path)**~~ — DONE: added `businessSettings.businessGstNumber` (migration `20260627_business_gst_number.sql`, TM seeded by name) + `getBusinessIdentity().gstNumber` (default `''`, **no TM fallback**). Branded-email footer now renders the GST line only when set, so a new tenant never shows TM's GST. **Still TODO:** PDF GST (`document_templates.gstNumber` path, `routes.ts:1334/1543`) + a Settings UI so subscribers can enter their own GST/owner/tagline (no business-identity settings form exists yet — see admin-defaults item above).
