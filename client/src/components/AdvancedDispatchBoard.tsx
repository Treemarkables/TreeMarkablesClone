...
export function AdvancedDispatchBoard({
  compact = false,
}: AdvancedDispatchBoardProps) {
  ...
  const allJobs = useMemo(() => {
    ...
    console.log("🔍 Dispatch Board Debug:", {
      ...
      top10Jobs: sorted
        .slice(0, 10)
        .map((j: any) => ({ jobNumber: j.jobNumber, title: j.title })),
    });
  ...
  console.log(
    "⚡ AdvancedDispatchBoard RENDER - Raw Jobs:",
    allJobsRaw?.length,
    "Sorted Jobs:",
    allJobs?.length,
  );
  ...
  const getCustomerName = (customerId: string, jobData?: any) => {
    ...
    const job =
      jobData ||
      (jobsData as any)?.data?.find((j: any) => j.customerId === customerId);
  ...
  const renderJobSidebarCard = (job: any, index: number) => {
    ...
    const scheduledDateText = job.scheduledDate
      ...
      return (
        ...
        >
          <div className="font-medium">
            {jobTime.getHours()}:
            {jobTime.getMinutes().toString().padStart(2, "0")}
          </div>
          <div className="truncate">{getCustomerName(job.customerId, job)}</div>
          {jobTotal > 0 && (
            <div className="font-bold text-[10px] opacity-90">
              {formatCurrency(jobTotal)}
            </div>
          )}
        </div>
  ...
  return (
    ...
    >
      <div
        ...
        data-testid="advanced-dispatch-board"
      >
            ...
            <div className="text-center">
              <span className="text-green-600 font-medium text-lg">
                ...
                <span className="text-sm ml-1">
                  {format(new Date(), "h:mmaaaa").toLowerCase()}
                </span>
              </span>
            </div>
          </div>
          ...
          <div className="w-72 bg-white border-l overflow-y-auto">
            ...
            <div className="p-3 pb-8 space-y-3">
              {allJobs.map((job: any, index: number) =>
                renderJobSidebarCard(job, index),
              )}
            </div>
          </div>
        </div>
