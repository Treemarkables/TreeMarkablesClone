...
export function PhotoDocumentation({
  ...
  const photosQuery = useQuery({
    queryKey: jobId
      ? ["job-photos", jobId, filterType]
      : ["all-photos", filterType],
    queryFn: async () => {
      let url: string;
      if (jobId) {
              ...
              <div className="grid grid-cols-3 gap-2">
                {recentPhotos.map((photo) => (
                  ...
                  >
                    ...
                    />
                    <div
                      className="w-full h-full bg-gradient-to-br from-green-200 to-blue-200 flex items-center justify-center"
                      style={{ display: "none" }}
                    >
                      <FileImage className="h-8 w-8 text-white" />
                    </div>
                    <Badge
                      className={`${getTypeColor(photo.type)} text-white absolute top-1 left-1 text-xs`}
                    >
      ...
      <Card>
        <CardHeader>
          ...
          <div className="flex gap-2 mt-4">
            ...
            {["before", "during", "after", "damage", "safety", "equipment"].map(
              (type) => {
                  ...
                  <CardContent className="p-3">
                    ...
                    <div className="space-y-1">
                      <h4
                        ...
                        data-testid={`job-title-${photo.id}`}
                      >
                        {photo.jobTitle}
                      </h4>
                      <p
                        className="text-xs text-muted-foreground"
                        data-testid={`customer-${photo.id}`}
                      >
                        {photo.customerName}
                      </p>
                      <p
                        className="text-xs text-muted-foreground"
                        data-testid={`date-${photo.id}`}
                      >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <h4
                            ...
                            data-testid={`list-job-${photo.id}`}
                          >
                            {photo.jobTitle}
                          </h4>
                          <Badge
                            className={`${getTypeColor(photo.type)} text-white`}
                          >
                            {getTypeText(photo.type)}
                          </Badge>
                        </div>
          ...
          <DialogContent className="max-w-4xl">
            <DialogHeader>
              <DialogTitle data-testid="photo-detail-title">
                {selectedPhoto.jobTitle} - {getTypeText(selectedPhoto.type)}{" "}
                Photo
              </DialogTitle>
            </DialogHeader>
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span>Type:</span>
                      <Badge
                        className={`${getTypeColor(selectedPhoto.type)} text-white`}
                      >
                    ...
                    <div className="flex justify-between">
                      ...
                      <span>
                        {format(
                          new Date(selectedPhoto.capturedAt),
                          "MMM dd, yyyy HH:mm",
                        )}
                      </span>
                    </div>
