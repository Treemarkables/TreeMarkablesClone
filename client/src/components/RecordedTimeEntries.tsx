...
interface RecordedTimeEntriesProps {
  jobId: string;
  jobNumber: string;
  /**
   * When true the component is rendered inline (e.g. inside the Back Costing
   * tab) rather than inside a modal. Drops the top padding that otherwise
   * clears the modal's floating close button, and a save no longer tries to
   * dismiss a parent dialog.
   */
  embedded?: boolean;
  /**
   * Called after the modal flows that close the dialog (save / add-to-invoice).
   * Optional so the embedded surface can leave the panel open — invalidated
   * queries refresh the labor totals in place.
   */
  onClose?: () => void;
  /**
   * Controls whether the staff-entry/expense forms are mounted. The modal
   * always mounts (it owns its own open/close); the embedded surface only
   * mounts when the surrounding panel decides to fetch. Defaults to true.
   */
  enabled?: boolean;
}
...
export function RecordedTimeEntries({
  jobId,
  jobNumber,
  embedded = false,
  onClose,
  enabled = true,
}: RecordedTimeEntriesProps) {
  ...
  // Refetch data when the surface (re)mounts / becomes enabled
  useEffect(() => {
    if (enabled && jobId) {
      refetchTimeEntries();
      refetchMaterials();
  ...
  }, [enabled, jobId, refetchTimeEntries, refetchMaterials, refetchEmployees]);
  ...
  const saveAllEntries = async () => {
    try {
      ...
      // Clear pending state so the embedded panel reflects the save without
      // a dialog dismiss; the modal surface dismisses via onClose.
      setPendingEntries([]);
      setExpenses([]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        className={`bg-blue-50 border border-blue-200 rounded-lg p-3 sm:p-4 space-y-3 ${
          embedded ? "" : "pt-10"
        }`}
      >
