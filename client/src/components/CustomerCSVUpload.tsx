...
export function CustomerCSVUpload() {
  const [uploadStep, setUploadStep] = useState<
    "upload" | "parsing" | "preview" | "updating" | "complete"
  >("upload");
  ...
  const toggleCustomerUpdate = (index: number) => {
    ...
    const newWillUpdateCount = updatedMatches.filter(
      (match) => match.willUpdate,
    ).length;
  ...
  const bulkUpdateMutation = useMutation({
    mutationFn: async (matches: CustomerMatch[]): Promise<BulkUpdateResult> => {
      return await apiRequest("POST", "/api/customers/bulk-update", {
        matches,
      });
    },
  ...
  const handleBulkUpdate = () => {
    ...
    const matchesToUpdate = matchingResult.matches.filter(
      (match) => match.willUpdate,
    );
    ...
    return (
      <div className="flex flex-col gap-1">
        ...
        {match.matchType !== "none" && (
          <Badge
            className={`text-xs ${confidenceColors[match.matchConfidence]}`}
          >
          ...
          <div className="mt-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  <CardContent className="p-4">
                    <div className="flex items-center space-x-2">
                      <div
                        className={`w-3 h-3 rounded-full border-2 ${importMode === "import" ? "bg-primary border-primary" : "border-muted-foreground"}`}
                      />
                      <div>
                        <div className="font-medium">Full Import</div>
                        <div className="text-sm text-muted-foreground">
                          Import new customers and update existing ones
                        </div>
                      </div>
                    </div>
                  </CardContent>
                ...
                >
                  <CardContent className="p-4">
                    <div className="flex items-center space-x-2">
                      <div
                        className={`w-3 h-3 rounded-full border-2 ${importMode === "update" ? "bg-primary border-primary" : "border-muted-foreground"}`}
                      />
                      <div>
                        <div className="font-medium">Update Only</div>
                        <div className="text-sm text-muted-foreground">
                          Update existing customer names only
                        </div>
                      </div>
                    </div>
                  </CardContent>
            ...
            <div>
              ...
              <ul className="text-sm text-muted-foreground space-y-1">
                <li>
                  • ServiceM8 Customer Export (company_name, contact_first,
                  contact_last, email, mobile, uuid)
                </li>
    ...
    return (
      <Card className="w-full" data-testid="card-csv-preview">
        <CardHeader>
          ...
          <CardDescription>
            Review the matches before applying updates.{" "}
            {matchingResult.willUpdateCount} customers will be updated.
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center p-3 bg-muted rounded-lg">
                <div className="text-2xl font-bold">
                  {matchingResult.totalRows}
                </div>
                <div className="text-sm text-muted-foreground">Total Rows</div>
              </div>
              <div className="text-center p-3 bg-muted rounded-lg">
                <div className="text-2xl font-bold">
                  {matchingResult.matchableRows}
                </div>
                <div className="text-sm text-muted-foreground">Matchable</div>
              </div>
              <div className="text-center p-3 bg-green-50 rounded-lg">
                <div className="text-2xl font-bold text-green-600">
                  {matchingResult.highConfidenceMatches}
                </div>
                <div className="text-sm text-muted-foreground">
                  High Confidence
                </div>
              ...
              <div className="text-center p-3 bg-blue-50 rounded-lg">
                <div className="text-2xl font-bold text-blue-600">
                  {matchingResult.willUpdateCount}
                </div>
                      ...
                      <TableRow key={index} data-testid={`match-row-${index}`}>
                        ...
                        <TableCell>
                          {match.existingCustomer ? (
                            <div className="text-sm">
                              <div className="font-medium">
                                {match.existingCustomer.name}
                              </div>
                              <div className="text-muted-foreground">
                                {match.existingCustomer.email}
                              </div>
                            </div>
                          ) : (
    ...
    return (
      <Card className="w-full" data-testid="card-csv-updating">
        <CardHeader>
          ...
          <CardDescription>Applying customer name updates...</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
        ...
        <CardContent>
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <div className="text-center p-3 bg-muted rounded-lg">
                <div className="text-2xl font-bold">
                  {bulkUpdateResult.updated + bulkUpdateResult.failed}
                </div>
                <div className="text-sm text-muted-foreground">
                  Total Processed
                </div>
            ...
            <Alert>
              ...
              <AlertDescription>{bulkUpdateResult.message}</AlertDescription>
            </Alert>
                ...
                <AlertDescription>
                  <div className="space-y-1">
                    ...
                    {bulkUpdateResult.errors.length > 5 && (
                      <div className="text-sm">
                        ... and {bulkUpdateResult.errors.length - 5} more
                      </div>
                    )}
                  </div>
                </AlertDescription>
