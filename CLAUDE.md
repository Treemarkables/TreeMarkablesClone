...
- Production runs on Digital Ocean App Platform (app `plankton-app`, region **SYD** — this file previously said SGP1, which was wrong; the app spec's `region: syd` is authoritative, verified via `doctl apps spec get` 2026-07-13). The app and the Neon DB (`ap-southeast-2`) are already co-located in Sydney — do NOT plan region migrations off this file without re-verifying against the spec. Every push to `main` triggers an autodeploy; for build/runtime logs check the DO dashboard. The web service runs `instance_count: 2` — background cron workers are gated per-APP (`RUN_CRONS`), not per-instance, so both instances run them (duplicate-send hazard, see WORK_REGISTRY).
