...
- **Email Composer Amount Formatting Fix**: Fixed runtime error in email composer where invoice amounts were being treated as strings instead of numbers. Created formatAmount() helper using Number() and Number.isFinite() to safely convert any value (string, number, null, undefined) to formatted currency, preventing $NaN outputs in emails. Applied to all invoice, quote, and proposal amount replacements in both initial template population and template switching.
- **Invoice Builder Editable Email Field**: Added editable email field to invoice builder, allowing users to change the recipient email address before sending invoices. Email field displays between address and description, pre-filled with customer's email, and passes edited value to email composer via customEmail prop.
- **Invoice Template Description Label**: Changed "Notes" section heading to "Description" in invoice template for clarity. Section displays both invoice notes and description together (joined with double newline).
- **Invoice Builder Dual-Button Workflow**: Added two-button workflow to invoice builder: (1) "Save Invoice" button saves invoice and closes dialog without opening email composer, (2) "Send Invoice" button saves invoice and immediately opens email composer for sending. Both buttons share validation logic and create invoice once, then diverge based on user intent (save-only vs send).
- **Invoice Builder Connected to Proposals/Quotes**: Invoice builder now automatically pulls line items and description from accepted or sent proposals/quotes. Smart fallback logic: proposal sections → quote lineItems → job lineItems → job total. Protected against user edit overwrites with hasInitialized flag ensuring data loads only once. UI reorganized: Description field moved to replace "Line Items" header with "Add Item" button inline. Invoice creation flow: Click Invoice → Loads proposal/quote data → Edit address/description/line items → Save or Send Invoice.
- **Invoice Builder with Editable Fields (CRITICAL)**: Replaced immediate invoice creation with split-screen invoice builder allowing users to edit address, line items, due date, and notes before finalizing invoices. Fixed critical bug where invoice totals weren't recalculated from edited line items. Backend now properly sums customData.lineItems when provided (falling back to job total when no items exist), and partial invoices apply percentage to computed base. Added address field to invoices table for storing service location with each invoice. Invoice viewer now displays stored invoice address instead of always using job address. Removed redundant "Proposal Overview" section from proposal builder. Invoice creation flow: Click Invoice → Opens InvoiceBuilder → Edit fields → Create Invoice button → Saves to database with correct totals.
