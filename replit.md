...
- **iOS PWA Tap-to-Record**: Fixed speech-to-quote to work natively on iOS PWA by testing actual MediaRecorder capability instead of blocking all iOS devices. Now supports tap-to-record with audio/mp4 format on iPhones where supported, with automatic fallback to upload for unsupported devices. Includes high-quality 128kbps recording with 48kHz sample rate, echo cancellation, and noise suppression for accurate transcription.
