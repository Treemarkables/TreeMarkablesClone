...
- **Invoices Page Clean Slate & Auto-Remove**: Archived 1,900 old completed jobs to provide clean invoices page. Jobs now automatically disappear from invoices page after sending to Xero (status changes to 'invoiced'). Only newly completed jobs appear in the invoices list.
- **Xero Integration Fix**: Fixed "Send to Xero" button error - corrected function name from `createJobDiary` to `createJobDiaryEntry` and removed duplicate endpoint. Xero invoices now send successfully with proper job diary logging.
- **Invoice Email Placeholder Fix**: Fixed email template placeholder replacement to show actual invoice data (invoice number, amount, address, due date) instead of {placeholders}. Uses global regex replacement to handle all instances in both subject and body.
- **Auto-Attach Invoice Fix**: Fixed invoice PDF auto-attachment in email composer - invoice now automatically appears in "Attached:" section without requiring manual selection from Smart Attachments popover.
