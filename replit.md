...
## Recent Changes
### October 18, 2025
- **Split-Screen Layout Implementation - PARTIAL**: Implemented ResizablePanelGroup in DispatchBoard with 60% left panel (dispatch board) and 40% right panel (job details). Split-screen only displays on desktop (lg: breakpoint), mobile continues using full-screen modal. **Current limitation**: Job cards still render as Dialog overlays on desktop instead of inline in the right panel due to GlobalJobCard component complexity (4000+ lines with Dialog wrapper creating portal overlays incompatible with panel rendering). Refactoring required to support both dialog and panel rendering modes.
- **Push Notification System - COMPLETE & TESTED**: Successfully built and tested complete Firebase Cloud Messaging infrastructure for real-time notifications. All components verified working in Safari browser. System includes: database schema (`fcm_tokens`, `notification_preferences`), storage methods, Firebase services (frontend/backend), service worker with retry logic and immediate initialization, API endpoints, and notification triggers. Notifications automatically sent for: job assignments, schedule changes, new leads, invoice payments, and quote acceptances. NotificationSettings UI (Settings → Notifications) allows users to enable notifications and manage preferences. Test notification feature confirmed working. Firebase credentials configured successfully.
