...
- **Invoice Builder Connected to Proposals/Quotes**: Invoice builder now automatically pulls line items and description from accepted or sent proposals/quotes. Smart fallback logic: proposal sections → quote lineItems → job lineItems → job total. Protected against user edit overwrites with hasInitialized flag ensuring data loads only once. UI reorganized: Description field moved to replace "Line Items" header with "Add Item" button inline, label changed from "Description / Notes" to "Description", and "Save Invoice" button replaces "Create & Send Invoice". Invoice creation flow: Click Invoice → Loads proposal/quote data → Edit address/description/line items → Save Invoice → Opens email composer.
