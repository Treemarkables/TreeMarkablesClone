...
## Recent Changes
### October 22, 2025
- **Invoice Email Embedded Display - IMPLEMENTED**: Changed invoice emails to send full invoice HTML directly in email body instead of sending view links. Removed "View & Pay Invoice Online" button from invoice HTML template in server/routes.ts (lines 4656-4672). Updated email template in database to remove {invoiceLink} placeholder and added message "Your invoice is shown below. You can make payment via bank transfer using the payment details provided." Customers now receive complete invoice immediately when opening email with all details visible: header with invoice number, Bill To section, description/line items, totals (Subtotal excl GST, GST 15%, Total Amount), payment information (ANZ bank details), and business footer. No links to click - invoice is fully embedded in email.
