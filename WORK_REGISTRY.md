...
## Active
- unified calendar — branch `feat/unified-calendar` — 2026-06-10 — rebuild `/calendar` as the single calendar surface in 3 PRs: (A) month/week/day views + staff/status filters + multi-day span bars, extracting CalendarGrid machinery into `client/src/components/calendar/` [PR #161]; (B) pointer-events drag-and-drop incl. touch + double-booking warnings (`shared/scheduleConflicts.ts`, opt-in 409 on staff-assignment routes) [PR #162]; (C) per-user two-way Google Calendar sync (3 new tables, push on job/assignment writes, RUN_CRONS pull poller). /dispatch CalendarGrid untouched behaviorally.
