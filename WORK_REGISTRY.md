...
## Active
- UI refine (settings + vehicle inspection) — branch `feat/ui-refine-settings-inspection` — 2026-06-15 — de-"AI-slop" pass, refined & calm direction (impeccable skill). Foundation: refine `--primary` 214/75/50 → calm slate-blue 222/47/45 (light) / 222/55/62 (dark) + matching --ring/--sidebar-primary (index.css); --blue + chart ramp left as literal blue. Settings.tsx: regroup 30 items into 6 labelled sections, kill rainbow pastel icon tiles → single neutral chip + compact left-aligned rows, add client-side search, drop dead "Quick Actions" block, tokens not gray-*. VehicleInspectionSettings.tsx: calmer header (no font-bold/3xl), calmer selected-template (full border not side-stripe), token-based checklist rows. NO display font in app UI (product-register ban). Header/App.tsx intentionally untouched. Stacked on `feat/compliance-expiry-reminders` (PR #182).
